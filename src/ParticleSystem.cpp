...
void ImpulseRadialForce::updateForce(Particle * particle, ofVec3f dir) {
...
bool ParticleSystem::onTallGround(ParticleSystem* ground){
    if ((particles[0].position.x+particles[0].width) >= ground->particles[1].position.x){
        if((particles[0].position.y+particles[0].height) >= ground->particles[1].position.y) {
            return true;
        }
    }
    else {
...
bool ParticleSystem::onLowGround(ParticleSystem* ground){
    if(onTallGround(ground)){
        if(particles[0].position.x+particles[0].width<=ground->particles[1].position.x &&
           particles[0].position.y+particles[0].height <= ground->particles[1].position.y){
            if(particles[0].position.y+particles[0].height >= ground->particles[0].position.y)
                return true;
        }
    }
    else {
